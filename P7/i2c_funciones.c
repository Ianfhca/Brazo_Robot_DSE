...
unsigned int StartI2C_1()
{
  ...
  I2C1CONbits.SEN = 1; //Generate Start Condition Start enable a 1
  time_out= 0;
...
********************************************************************/
unsigned int WriteI2C_1(unsigned char byte) //byte es la direccion del esclavo (8 bits)
{
 unsigned int time_out;
 I2C1TRN = byte; //Load byte to I2C1 Transmit buffer
 time_out= 0;
 //wait for data transmission
 //TBF bit de registro de estado, indica que la transmision ha terminado
...
unsigned int ACKCheck_1(unsigned int *ACKstatus) //
{
	...
	time_out= 0;
    //wait for ACK
    //TRSTAT, registro de estado relacionado al ACK
    //El bit se pone a cero cuando el receptor envia el ACK
    while (I2C1STATbits.TRSTAT && (time_out<1000)) time_out++;
    if(time_out>=1000) return 1;
...
unsigned int getI2C_1(unsigned char *byte)
{
 ...
 //Queda esperando a que el bit de registro de estado RBF indique que ya se ha recibido un dato
